...
public class TermVector {
  ...
  private Term[] terms;
  private Map<String, Integer> stemMap;
  private long normalize_length; // normalize length sum(stemFreq * stemFreq)
  ...
  public TermVector(int docId, String fieldName) throws IOException {
    ...
    for (int i = 1; ithTerm.next() != null; i++) {
      ...
      normalize_length += stemsFreq[i] * stemsFreq[i];
...
/*    for (int s=1; s<this.stems.length; s++)
  ...
  /**
   * Returns the normalize_length of this document
   * @return
   */
  public long getNormalizeLength () {
    return this.normalize_length;
  }
