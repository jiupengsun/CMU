...
public class QryParser {
  ...
  private static Qry parseString (String queryString)
    ...
    boolean isWeightOp = queryTree instanceof QrySopWeight;
...
/*    if (queryTree instanceof QrySopWeight) {
      // process query string
      // retrieve substring between left ( and right )
      String tmpString = queryString.substring(0, queryString.lastIndexOf(')')).trim();
      String[] params = tmpString.split(" ");
      int length = params.length;
      if ((length % 2) != 0)
        syntaxError("Wrong format of WAND operator!");
      StringBuilder sb = new StringBuilder();
      for (int i=0; i<length; i+=2) {
        float weight = Float.parseFloat(params[i]);
        ((QrySopWeight) queryTree).addWeight(weight);
        params[i] = "";
      }
      sb.append(" ");
      sb.append(String.join(" ", params));
      sb.append(" " + queryString.substring(queryString.indexOf(')')));
    ...
    while (queryString.length() > 0) {
      ...
      if (queryString.charAt(0) == '#') {	// Subquery
        ...
        if (isWeightOp && last_weight >= 0) {
          ((QrySopWeight) queryTree).addWeight(last_weight);
          last_weight = -1f;
        }
      } else {					// Term
        ...
        if (!isWeightOp)
          ...
          if (last_weight < 0)
            last_weight = Float.parseFloat(p.getPopped());
          else {
            qargs = createTerms(p.getPopped());
            if (qargs.length > 0) {
              // not stopped word
