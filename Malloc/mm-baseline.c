...
#ifdef NEXT_FIT
    rover = heap_listp; // Sets rover at first block header
#endif
...
#ifdef NEXT_FIT
    // Adjust the rover so that it won't point to recently-coalesced free block
    if ((rover > block) && (rover < find_next(block))) 
    {
        rover = block;
    }
#endif
...
#ifdef NEXT_FIT
    // Next fit search
    block_t *oldrover = rover;
    // Search from the rover to the end of list
    for ( ; get_size(rover) > 0; rover = find_next(rover))
        if (!(get_alloc(rover)) && (asize <= get_size(rover)))
        {
            return rover;
        }

    // search from start of list to old rover
    for (rover = heap_listp; rover < oldrover;
                             rover = find_next(rover))
        if (!(get_alloc(rover)) && (asize <= get_size(rover)))
        {
            return rover;
        }
